...
L'implementazione del server \gls{DNS} di base segue uno sviluppo molto modulare sfruttando vari componenti che ne consento la facile estensione e modifica.
...
Ciò consente di rendere il thread di ascolto sempre reattivo, in quanto viene scaricato di qualsiasi lavoro di gestione della richiesta, minimizzando i tempi di reazione alla ricezione.
...
Sebbene si sia parlato del possibile sviluppo di un'interfaccia grafica multi-piattaforma, in realtà il prototipo di ClientShield utilizza la libreria \gls{WPF}, specifica per ambienti Windows.
La scelta è dovuta principalmente ad una maggiore conoscenza pregressa della libreria.
...
L'idea dietro quest'implementazione non è soltanto di facilitare uno sviluppo multi-platform, ma anche di garantire la modifica dell'interfaccia con il minimo intervento.
...
Come intuibile, l'applicazione è strutturata su tre schermate principali, seguendo la logica illustrata in figura \ref{fig:schema-casi-uso}:
\begin{itemize}
...
All'avvio dell'applicazione si connette come client sulla named pipe dedicata a ClientShield, permettendo l'invio dei comandi relativi alla regisrazione, autorizzazione e verifica dello stato, occupandosi della serializzazione e deserializzazione dei messaggi.
