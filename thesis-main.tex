...
\makeatletter
\renewcommand{\AC@placelabel}[1]{} % avoid acronym label before definition
\makeatother
...
\begin{itemize}
	\item \textbf{A livello di rete:} applicato direttamente dal \ac{ISP}, blocca l'accesso a domini malevoli prima che le richieste raggiungano la rete locale.  
	\newline \textit{Esempio: un'azienda utilizza un servizio DNS filtrato fornito dal proprio ISP per impedire l'accesso a siti di phishing su tutta la rete aziendale.}
	
	\item \textbf{A livello di gateway (router):} il router funge da punto di controllo, utilizzando un server DNS con filtraggio per impedire l'accesso a contenuti non autorizzati. Tutti i dispositivi connessi alla rete beneficiano automaticamente della protezione.  
