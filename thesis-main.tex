...
Il filtraggio \ac{DNS} può essere implementato in diverse modalità:

\begin{itemize}
	\item \textbf{A livello di rete:} applicato direttamente dal \ac{ISP}~\ref{isp-footnote}, blocca l'accesso a domini malevoli prima che le richieste raggiungano la rete locale.  
	\newline \textit{Esempio: un'azienda utilizza un servizio \ac{DNS} filtrato fornito dal proprio \ac{ISP} per impedire l'accesso a siti malevoli su tutta la rete aziendale.}
	
	\item \textbf{A livello di router:} il router funge da punto di controllo, utilizzando un server DNS con filtraggio per impedire l'accesso a contenuti non autorizzati. Tutti i dispositivi connessi alla rete beneficiano automaticamente della protezione.  
	\newline \textit{Esempio: un istituto scolastico configura il proprio router con OpenDNS per bloccare siti per adulti e social network sugli studenti.}
	...
	\newline \textit{Esempio: un dipendente in smart working utilizza un client DNS sicuro sul proprio laptop per proteggersi dalle minacce mentre lavora da reti Wi-Fi pubbliche.}
\end{itemize}

\footnote{
	\label{isp-footnote}
	Un \ac{ISP} è un fornitore che offre connettività alla rete, consentendo ai suoi utenti di accedere a Internet. Gli ISP possono anche fornire servizi aggiuntivi, come, appunto, sicurezza informatica e filtraggio.}
...
Allo stato dell'arte il prodotto utilizza una \acf{VPN}, attraverso cui il software genera un "tunnel \ac{DNS}" sicuro, che instrada le richieste effettuate dall'endpoint attraverso la rete aziendale.
Ciò accade perché la \ac{VPN} consente di far apparire l'endpoint remoto come connesso alla rete locale aziendale, mascherandone l'indirizzo IP e consentendogli dunque di utilizzare il \ac{DNS} aziendale per applicare le stesse strategie di filtraggio.

L'esigenza di aggiornamento del software deriva dall'allineamento di FlashStart al nuovo protocollo \acf{DoH}, descritto in \cite{RFC8484}.
Tale protocollo, infatti, si presta meglio ad un'azienda che offre filtraggio \ac{DNS} rispetto ad una classica \ac{VPN}. \ac{DoH} garantisce la riservatezza sfruttando la crittografia asimmetrica implementata dal protocollo \ac{HTTPS}, per garantire autenticazione, confidenzialità e integrità delle richieste \ac{DNS}.
