...
\subsection{Gestione della protezione}

L'avvio della protezione è un problema più complesso di quanto possa sembrare.
Per attivarla è necessaria l'interazione con molti componenti del sistema: il server name pipe dovrebbe gestire le API FlashStart, consentendo registrazione, richieste di autorizzazione e controlli periodici di stato del servizio, dovrebbe inoltre verificare lo stato del server \gls{DNS} e interagire con esso, permettendo di modificarne l'esecuzione. 
...
Questo pattern permette la creazione di una semplice interfaccia che consenta la gestione di funzionalità appartenenti a più classi.

La struttura delle classi viene illustrata in figura \ref{fig:protezione-facade-pattern}, il server named pipe ha il solo compito di ricevere ed elaborare le richieste da parte del client, delegando l'esecuzione di qualsiasi operazione alla classe ProtectionManager.
Quest'ultima funge da punto di raccordo tra le diverse funzionalità offerte dal software, gestendole in maniera organizzata e permettendo una granularità maggiore, che facilità la manutenzione.
\begin{figure}[H]
	\centering
...
\end{figure}
ProtectionManager coordina l'esecuzione delle funzionalità offerte dalle classi gestite, consentendo una gestione implicita delle credenziali (il codice utente non verrà mai utilizzato dal server pipe) e verificando la coerenza tra gli stati dei componenti tra loro.
Quando instanziato, ProtectionManager verificherà la presenza di una sessione attiva e, in tal caso, avvierà automaticamente la protezione.
