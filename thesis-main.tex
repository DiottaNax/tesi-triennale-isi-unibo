...
Il filtraggio \gls{DNS} può essere implementato in diverse modalità:
...
Allo stato dell'arte il prodotto utilizza una \gls{VPN}, attraverso cui il software genera un "tunnel \gls{DNS}" sicuro, che instrada le richieste effettuate dall'endpoint attraverso la rete aziendale.
Ciò accade perché la \gls{VPN} consente di far apparire l'endpoint remoto come connesso alla rete locale aziendale, mascherandone l'indirizzo IP e consentendogli dunque di utilizzare il \gls{DNS} aziendale per applicare le stesse strategie di filtraggio.

L'esigenza di aggiornamento del software deriva dall'allineamento di FlashStart al nuovo protocollo \gls{DoH}, descritto in \cite{RFC8484}.
Tale protocollo, infatti, si presta meglio ad un'azienda che offre filtraggio \gls{DNS} rispetto ad una classica \gls{VPN}. \gls{DoH} garantisce la riservatezza sfruttando la crittografia asimmetrica implementata dal protocollo \gls{HTTPS}, per garantire autenticazione, confidenzialità e integrità delle richieste \gls{DNS}.
